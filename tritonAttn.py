...
try:
    ...
    try:
        # 如果找不到triton，尝试查找triton-windows
        triton_version = version('triton-windows')
    except:
...
def forward_kernel_causal_and_sparse(
    ...
    if INCLUDE_BLOCK_CAUSAL:
        ...
        for kv_block_offset_ind in range(num_kv_blocks):
            ...
            # 优化后的softmax计算
            m_i_scaled = m_i / softmax_scale  # 预先缩放m_i以减少计算
            row_max = tl.max(qk, 2)  # [BLOCK, QUERY_HEAD_GROUPS]
            m_ij = tl.maximum(row_max, m_i_scaled) * softmax_scale  # 使用预先计算的值

            # 直接计算缩放后的qk，减少一次内存访问
            qk_scaled = qk - row_max[:, :, None]  # 先减去最大值提高数值稳定性
            p = tl.exp(qk_scaled * softmax_scale)  # 然后应用softmax_scale

            # 使用fused操作计算softmax，减少内存访问
            row_sum = tl.sum(p, 2) + 1e-10  # 增大epsilon值提高数值稳定性
            p = p / row_sum[:, :, None]  # 归一化
            ...
            #lse_i = m_ij + tl.log(l_i_new)修改
            lse_i = tl.clamp(m_ij + tl.log(l_i_new), -1e4, 1e4)
